...
function ek_update(iteration_::Int64)
    ...
    # TODO WRITE EQUIV METHOD FOR MiMA OUTPUT
    ...
    t0_ = 0.0
    tf_ = 1800.0
    # Get observations (MiMA)
    #yt = zeros(0)
    #yt_var_list = []
    #y_names = ["u", "v"]
    # TODO WRITE METHOD FOR GETTING QBO METRICS
    #yt_, yt_var_ = get_mima_metrics("truth_output", y_names, ti = t0_, tf = tf_, get_variance = true)
    ...
    # Get outputs
    g_names = y_names
    g_ = get_mima_metrics("$(versions[1]).output", g_names, ti = t0_, tf = tf_)
    g_ens = zeros(length(versions), length(g_))
    for (ens_index, version) in enumerate(versions)
        g_ens[ens_index, :] = get_mima_metrics("$(version).output", g_names, ti = t0_, tf = tf_)
    end
    g_ens = Array(g_ens')
    # Construct EKP
